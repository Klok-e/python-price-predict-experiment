...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Directly download Binance USD-M premium index kline CSV files.")
...
def download_one(ticker, interval, day, output_dir, retries, sleep_seconds):
    filename = f"{ticker}-{interval}-{day.isoformat()}.csv"
    output_path = os.path.join(output_dir, filename)
    if os.path.exists(output_path):
        return "cached"

    url = f"{BASE_URL}/{ticker}/{interval}/{ticker}-{interval}-{day.isoformat()}.zip"
    last_error = None
...
def main(argv=None):
    ...
    for ticker in tickers:
        ...
        for day in date_range(start, end):
            status = download_one(ticker, args.interval, day, output_dir, args.retries, args.sleep)
